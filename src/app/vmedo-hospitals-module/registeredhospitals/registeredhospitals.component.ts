...
@Component({
  selector: 'app-registeredhospitals',
  templateUrl: './registeredhospitals.component.html',
  styleUrls: ['./registeredhospitals.component.css']
})
export class RegisteredhospitalsComponent implements OnInit {
  ...
  constructor(private adminservice: ServicesService) {
    ...
    this.adminservice.getHospitalListComplete().subscribe(x => {
      this.dataSource.data = x['objret']?.filter(m => m.approvalStatus === 1) as HospitalProperties[];
    });
