...
@Component({
  selector: 'app-rejectedhospitals',
  templateUrl: './rejectedhospitals.component.html',
  styleUrls: ['./rejectedhospitals.component.css']
})
export class RejectedhospitalsComponent implements OnInit {
  ...
  constructor(private adminservice: ServicesService) {
    ...
    this.adminservice.getHospitalListComplete().subscribe(x => {
      this.dataSource.data = x['objret'].filter(m => m.approvalStatus === 2) as HospitalProperties[];
    });
