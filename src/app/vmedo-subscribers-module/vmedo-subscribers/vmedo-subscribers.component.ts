...
@Component({
  ...
  templateUrl: './vmedo-subscribers.component.html',
  styleUrls: ['./vmedo-subscribers.component.css']
})
export class VmedoSubscribersComponent implements OnInit {
    ...
    this.http.get<any>(apiUrl).subscribe(
      (response) => {
        if (response.statusCode === 400 && response.message === 'No Data Found') {
          ...
          const searchResult = response.objret as SubscriberData[];
  ...
  fetchUsers(pageNumber: number) {
    ...
    this.http.get<any>(apiUrl).subscribe((response) => {
      this.dataSource.data = response.objret as SubscriberData[];
